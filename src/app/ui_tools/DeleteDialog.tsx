...
const DeleteDialog = ({ children, user }) => {
    ...
    const handleDeleteRoom = async (e:any) => {
        ...
        const response = await axios.delete('/room/delete-room', {
            headers: {
    ...
    return (
        ...
        <AlertDialogContent className={`${isMobile? 'text-center': ''} w-[80%] rounded-md`}>
            <AlertDialogHeader>Are you should that you want to delete this room?</AlertDialogHeader>
            <AlertDialogDescription className='flex justify-center flex-col items-center'>
                <p>This action cannot be undone. This will permanently delete all data assosiated in this room.</p>
                <div className='flex flex-col mt-8 gap-2 w-[60%]'>
                    ...
                    {!deleteAction ? '':
                    <form onSubmit={handleDeleteRoom} className='text-center'>
                        <p className='text-center'>Please the room name:</p>
